...
<head>
	...
	<title>Deposit base eGOLD</title>
...
<body>
	<div id="egold_menu_reg">
		<span id="egold_reg_card">
			<input class="egold_card" type="text" maxlength="25" placeholder="Enter data">
			<input id="egold_button_reg_pin" type="button" value="Add Data">
		</span>
	</div>
	<div id="egold_qr"></div>
...
$('#egold_button_reg_pin').on('click', function() {
	...
	if($('.egold_card').val().length>=10 && $('.egold_card').val().length<=25){
		...
		.done(function(json1) {
			...
			}else if(json_decode.error=="user_ip")$('#egold_error').html("Try again");
			else if(json_decode.error=="timeout")$('#egold_error').html("So often you can not register");
			else if(json_decode.error=="details")$('#egold_error').html("Incorrectly populated data");
			else $('#egold_error').html(json_decode.error);
		}, "json")
		.fail(function() {
			$('#egold_error').html("Unable to connect to server");
		})
		.always(function() {
		...
		$('#egold_button_reg_pin').removeAttr('disabled');
		$('#egold_error').html("Incorrect input field");
		$('#egold_error').fadeIn();
	}
});
</script>
