...
//Проверить работу аддона можно, запустив index.php файл открыв его URL в адресной строке браузера: http://[сайт]/egold_reg/index.php
//Чтобы встроить форму автоматической регистрации кошелька под своим кошельком в страницу сайта, нужно в тегах разместить код из index.php так, чтобы было:
//1. <head><script src="/egold_reg/js/jquery-3.2.1.min.js"></script><style>[код стиля]</style></head>
//2. <body>[html код форм и кнопок]</body>
//3. <script>[код скрипта]</script>

$password_bdview= "";//Пароль для просмотра страницы с результатами (например: 3teRl@Uk3): [сайт]/egold_reg/bdview.php?password=[пароль]
...
$period_clean= 7;//Период в днях через который удаляются записи без созданных кошельков, которые запросили пинкод на почту. Очищение всех записей осуществляется, если они старше заданного количества дней при запросе пинкода любым обращением к скрипту.
