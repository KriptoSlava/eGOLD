...
if($stop==1){
	...
	if(isset($sqltbl['value']))$json_arr['transactionscount']= $sqltbl['value'];
	...
	if(isset($sqltbl['date']))$json_arr['datelasttransaction']= $sqltbl['date'];
...
if($stop!=1){
		...
		&& signcheck($wallet['wallet'].($sqltbl_arr['signpubreg'] && $sqltbl_arr['signreg']?'00':$sqltbl_arr['recipient']).$sqltbl_arr['money'].$sqltbl_arr['pin'].$sqltbl_arr['height'].$sqltbl_arr['nodause'].($sqltbl_arr['signpubreg'] && $sqltbl_arr['signreg']?$sqltbl_arr['signpubreg'].$sqltbl_arr['signreg']:'').($sqltbl_arr['signpubnew'] && $sqltbl_arr['signnew']?$sqltbl_arr['signpubnew'].$sqltbl_arr['signnew']:''),$sqltbl_arr['signpub'],$sqltbl_arr['sign'])==1){
			...
			if($sqltbl_arr['checkhistory']==0 && $wallet_balance>=0){
				...
				if(mysqli_affected_rows($mysqli_connect)>=1){
					...
					query_bd("UPDATE `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` SET `percent_ref`=`percent_ref`+1 ".(mt_rand(1,100)==1?', `view`=IF(`view`=1,3,`view`)':'')." WHERE `wallet`='".$sqltbl_arr['nodawallet']."';");
  ...
  function wallet_synch($post_synchwallets){
    global $stop,$json_arr,$sqltbl,$mysqli_connect,$noda_ip,$checkbalancenodatime,$noda_balance_noda_ip,$noda_balance_count,$noda_balance,$limit_synch,$wallet_synch_end,$noda_trust,$nodas_balance_sum_bd;
      ...
      if(is_array($noda_json_arr) && count($noda_json_arr)>=3){
        ...
        foreach($noda_json_arr as $key1 => $value1){
								...
								if(isset($value1['synchwallets'])){
									foreach($value1['synchwallets'] as $key2 => $value2){
										if(isset($value2['wallet']) && (int)$value2['wallet']==$value2['wallet'] && strlen($value2['wallet'])==18 
											 ...
											 && isset($value2['view']) && (int)$value2['view']==$value2['view'] && strlen($value2['view'])==1 && $value2['view']>0
												){
												if(!isset($balance_check[$value2['wallet']]) || $balance_check[$value2['wallet']]<=$nodas_balance_sum/2){
													...
													$balance_check_view= [0,0,0];
													foreach($noda_balance_rand as $key3 => $value3){
														if(isset($noda_json_arr[$key3]['synchwallets'])){
															$checkTrue= 0;
															if(in_array($value_test1,$noda_json_arr[$key3]['synchwallets'])){
																$balance_check_view[0]+= $noda_balance_rand[$key3];$checkTrue=1;
															} else if(in_array($value_test3,$noda_json_arr[$key3]['synchwallets'])){
																$balance_check_view[2]+= $noda_balance_rand[$key3];$checkTrue=1;
															} else if(in_array($value_test2,$noda_json_arr[$key3]['synchwallets'])){
																$balance_check_view[1]+= $noda_balance_rand[$key3];$checkTrue=1;
															}
															if($checkTrue== 1)$balance_check[$value2['wallet']]+= $noda_balance_rand[$key3];
														}
													}
													...
													$wallet_add['view']=array_keys($balance_check_view, max($balance_check_view))[0]+1;
													if(isset($post_synchwallets['synch_wallet']) && $synch_wallet<$value2['wallet'])$synch_wallet=$value2['wallet'];
          ...
          if($wallet_synch_end>0){
            ...
            while($sqltbl_arr= mysqli_fetch_array($result,MYSQLI_ASSOC)){
							$nodas_balance_sum_bd_test_null= ($nodas_balance_sum_bd-($sqltbl_arr['view']==0?$noda_balance_noda_ip:0));
              if(((int)$sqltbl_arr['checkbalanceall']+$nodas_balance_sum)>=0.9*$nodas_balance_sum_bd_test_null && ((int)$sqltbl_arr['checkbalance']+($sqltbl_arr['view']>1 && (int)$sqltbl_arr['checkbalance']==0 && (int)$sqltbl_arr['checkbalanceall']==0?$noda_balance_noda_ip:0))<=0.5*$nodas_balance_sum_bd_test_null){
          ...
          if(isset($noda_json_arr_all_wallets) && $noda_json_arr_all_wallets){
						...
						while($sqltbl_arr= mysqli_fetch_array($result,MYSQLI_ASSOC)){
							if(isset($noda_json_arr_all_wallets[$sqltbl_arr['wallet']])){
								...
								if(isset($noda_json_arr_all_wallets[$sqltbl_arr['wallet']]['wallet'])
									...
									if(((int)$sqltbl_arr['checkbalance']+$balance_check[$sqltbl_arr['wallet']]+($sqltbl_arr['view']>1 && (int)$sqltbl_arr['checkbalance']==0 && (int)$sqltbl_arr['checkbalanceall']==0?$noda_balance_noda_ip:0))>0.5*$nodas_balance_sum_bd_test_null){
									...
									} else if(((int)$sqltbl_arr['checkbalanceall']+$nodas_balance_sum)>=0.9*$nodas_balance_sum_bd_test_null){
						...
						if(isset($noda_json_arr_all_wallets) && count($noda_json_arr_all_wallets)>0){
							...
							if(isset($noda_json_arr_all_wallets) && $noda_json_arr_all_wallets){
								foreach($noda_json_arr_all_wallets as $key => $value){
									if($value['view']==1)$wallets_bd_add[$value['wallet']]= "'".$value['wallet']."','".$value['ref1']."','".$value['ref2']."','".$value['ref3']."','".$value['noda']."','".$value['nodause']."','".$value['balance']."','".$value['date']."','".$value['percent_ref']."','".$value['date_ref']."','".$value['height']."','".$value['signpubnew']."','".$value['signnew']."','".$value['signpub']."','".$value['sign']."','".$balance_check[$value['wallet']]."','".$nodas_balance_sum."','".$checkbalancenodatime."','0'";
								}
								if(isset($wallets_bd_add)){
...
if(isset($_REQUEST['type']) && $_REQUEST['type']=="walletscount"){
	query_bd("SELECT count(*) as walletscount FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` WHERE `view`>0 LIMIT 1;");
...
if($stop!=1 && $request['type']=="history"){
	if((!isset($request['history']) && !isset($request['wallet']) && !isset($request['recipient']) ) || (isset($request['history']) && strlen($request['history'])==18) || (isset($request['wallet']) && strlen($request['wallet'])==18) || (isset($request['recipient']) && strlen($request['recipient'])==18)){
		...
		if(isset($request['history']) && strlen($request['history'])==18){
			...
			$query= "SELECT `wallet`, `recipient`, `money`, `pin`, `height`, `nodawallet`, `nodause`, `nodaown`, `date`, `signpubreg`, `signreg`, `signpubnew`, `signnew`, `signpub`, `sign`, `checkhistory` FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_history` WHERE ".$where." and `wallet`!=1 ORDER BY `date` ".$order." LIMIT ".$start.",".$limit.";";
			$result= mysqli_query($mysqli_connect,$query) or die("error_history");
			while($sqltbl_arr= mysqli_fetch_array($result,MYSQLI_ASSOC)){
				if($sqltbl_arr['wallet'])$json_arr['history'][$sqltbl_arr['date']]= $sqltbl_arr;
			}
...
if($stop!=1 && ($request['type']=="synch" || $request['type']=="send")){
  ...
  if($request['type']=="synch" || (!file_exists($GLOBALS['filename_tmp_synch']) && (int)date("s",$json_arr['timer_start'])>5)){
    $query= "SELECT * FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_settings` WHERE `name` IN ('synch_now','synch_wallet','check_wallet');";
    ...
    while($sqltbl_arr= mysqli_fetch_array($result,MYSQLI_ASSOC)){
      if($sqltbl_arr['name']=='synch_now' && date("Y-m-d H:i",$sqltbl_arr['value'])==date("Y-m-d H:i",$json_arr['timer_start'])){
        if($request['type']!="send"){echo '{"synch":"now"}';exit_now();}
				...
				$check_wallet_count= mt_rand(10,50);
				$max_wallet= 0;
				$query_wallet= "SELECT `wallet` FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` WHERE `wallet` > '".$sqltbl_arr['value']."' ORDER by `wallet` ASC LIMIT ".$check_wallet_count.";";
				$result_wallet= mysqli_query($mysqli_connect,$query_wallet) or die("error_noda_synch_for_last");
				while($sqltbl_arr_wallet= mysqli_fetch_array($result_wallet,MYSQLI_ASSOC))if($max_wallet<$sqltbl_arr_wallet['wallet'])$max_wallet= $sqltbl_arr_wallet['wallet'];
				if($max_wallet>0){
	...
	if($skip!=1){
		...
		query_bd("SELECT SUM(`balance`) as balanceall FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` WHERE `view`>0 LIMIT 1;");
		...
		delay_now();
		$query= "SELECT `wallet` FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` WHERE (`view`!=1 and ((`checkwallet`!= '' and `checkwallet`< ".strtotime(date("Y-m-d H:i:00",$json_arr['time']))."-16*24*60*60 and `checkwallet`>0) or (`view`=0 and `checkbalanceall`!='' and `checkbalanceall`> ".(0.9*($nodas_balance_sum_bd- $noda_balance_noda_ip))." and (`checkbalance`='' or `checkbalance`< 0.5*`checkbalanceall`)))) or (`date`< ".strtotime(date("Y-m-d H:i:00",$json_arr['time']))."-90*24*60*60 and (`balance`<10 or `date`< ".strtotime(date("Y-m-d H:i:00",$json_arr['time']))."-10*365*24*60*60));";
		$result= mysqli_query($mysqli_connect,$query) or die("error_wallets_dell");
		while($sqltbl_arr= mysqli_fetch_array($result,MYSQLI_ASSOC))query_bd("DELETE FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` WHERE `wallet`= '".$sqltbl_arr['wallet']."';");
		if((int)date("i",$json_arr['timer_start'])==51){
			...
			if(isset($history_size) && (int)$history_size>=1){
				...
				if(isset($sqltbl['count']) && $sqltbl['count']>$history_size){
					...
					query_bd("DELETE FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_history` WHERE `date`< ".strtotime(date("Y-m-d H:i:00",$json_arr['time']))."-1*24*60*60 ORDER by `date` ASC ".($history_size_extra>0?"LIMIT ".$history_size_extra:'').";");
					if(mysqli_affected_rows($mysqli_connect)>=1)delay_now();
				}
			}
			...
			query_bd("DELETE FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_users` WHERE `date`< ".strtotime(date("Y-m-d H:i:00",$json_arr['time']))."-365*24*60*60;");
			if(mysqli_affected_rows($mysqli_connect)>=1){
				delay_now();
		...
		if((int)date("H",$json_arr['timer_start'])<=5 && (int)date("H",$json_arr['timer_start'])>=3 && mt_rand(1,3*60)==1){
			query_bd("OPTIMIZE TABLE `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_contacts`, `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_history`, `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_referrals`, `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_settings`, `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_users`, `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets`;");
		}
	}  
  ignore_user_abort(0);
} 
...
if($stop!=1 && $request['type']=="synchwallets"){
  ...
  if(isset($_POST['wallets'])){
    ...
    if(is_array($wallets_temp) && count($wallets_temp)>0){
      ...
      if(isset($wallets)){
        $query= "SELECT `wallet`, `ref1`, `ref2`, `ref3`, `noda`, `nodause`, `balance`, `date`, `percent_ref`, `date_ref`, `height`, `signpubnew`, `signnew`, `signpub`, `sign`, `view` FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` WHERE `wallet` IN ('".implode("','",$wallets)."') and `view`>0 ORDER BY `date`,`wallet` LIMIT ".$limit.";";
  ...
  if(isset($request['synch_wallet']) && $request['synch_wallet']>=0){
    ...
    $query_wallets_synch= "SELECT `wallet`, `ref1`, `ref2`, `ref3`, `noda`, `nodause`, `balance`, `date`, `percent_ref`, `date_ref`, `height`, `signpubnew`, `signnew`, `signpub`, `sign`, `view` FROM `".$GLOBALS['database_db']."`.`".$GLOBALS['prefix_db']."_wallets` WHERE `wallet`>'".$request['synch_wallet']."' ".(isset($json_arr['synchwallets']) && count($json_arr['synchwallets'])>0?"and `wallet` NOT IN ('".implode("','",array_keys($json_arr['synchwallets']))."')":'')." and `view`>0 and `date`< UNIX_TIMESTAMP()-60 ORDER BY `wallet` LIMIT ".$limit.";";
