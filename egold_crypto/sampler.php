...
const half_gaussian_cdt = [
	['0x55252c92c6309bbb', '0xeff1ff6ad56fd4b1'],
...
function sampler_half_gaussian()
{
	...
	$u0 = gmp_random_range('0', '0xffffffffffffffff');
    $z = 0;
	$p0 = gmp_init(half_gaussian_cdt[$z][0]);
	
    // Scan the CDF table for the index where
    // the uniform deviate x0 exceeds the value of the CDF
    while(gmp_cmp($u0,$p0) > 0) {
        $z += 1;
		$p0 = gmp_init(half_gaussian_cdt[$z][0]);
	}
